...
var getStaff = function(id){
 ...
 var renderStaff = function(){
  // data = data || staffList;
  console.log(staffList[0].totalSalary());
    var dataHTML = "";
    for(var i = 0; i < staffList.length; i++){
       dataHTML += `<tr>
          ...
          <td>${staffList[i].workDay}</td>
          <td>${staffList[i].position}</td>
          <td>${staffList[i].totalSalary()}</td>
          <td>${staffList[i].type()}</td>
          <td>
             <button class = "btn btn-danger" onclick = "deleteStaff('${staffList[i].id}')">Xóa</button>
...
var validate = function () {
   ...
   var passPattern = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{6,10}$/g;
   ...
   isValid &= require(pass, "tbMatKhau") && pattern(pass, "tbMatKhau", passPattern);
